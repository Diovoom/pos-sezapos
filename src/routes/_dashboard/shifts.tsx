...
type TimeEntry = {
  ...
  late: boolean;
  late_minutes: number;
  profiles?: { full_name: string | null; first_name: string | null; last_name: string | null; employee_id: string | null; email: string | null } | null;
};

type Sale = { id: string; cashier_id: string | null; created_at: string; total: number; status: string };
...
function ShiftsList() {
  ...
  const employeesQ = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select("id, full_name, first_name, last_name, employee_id, email").order("full_name");
      return data ?? [];
    },
  });
  ...
  const exportCsv = () => {
    const header = ["Shift ID", "Employee ID", "Employee", "Date", "Clock In", "Clock Out", "Break (min)", "Hours", "Overtime", "Status", "Sales", "Transactions"];
                ...
                <div className="space-y-1">
                  ...
                  <Select value={employeeId} onValueChange={setEmployeeId}>
                    ...
                    <SelectContent>
                      ...
                      {(employeesQ.data ?? []).map((p: { id: string; full_name: string | null; email: string | null }) => (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {rows.map((r) => {
                      ...
                      const name = r.profiles?.full_name || `${r.profiles?.first_name ?? ""} ${r.profiles?.last_name ?? ""}`.trim() || r.profiles?.email;
                      return (
                        <tr key={r.id} className="border-b last:border-0">
                          ...
                          <td>
                            {r.profiles ? (
                              <Link to="/employees/$id" params={{ id: r.user_id }} className="hover:underline">
                                {name} <span className="text-xs text-muted-foreground font-mono">{r.profiles.employee_id}</span>
                              </Link>
                            ) : name}
                            {r.late && <Badge variant="outline" className="ml-2 border-warning text-warning">Late {r.late_minutes}m</Badge>}
                          </td>
                          <td>{format(inD, "p")}</td>
                          <td>{outD ? format(outD, "p") : <span className="text-primary">—</span>}</td>
                          <td className="text-right tabular-nums">{r.break_minutes ?? 0}m</td>
                          <td className="text-right tabular-nums font-medium">{(r.workedMin / 60).toFixed(2)}</td>
                          <td className={`text-right tabular-nums ${r.overtimeMin > 0 ? "text-warning" : ""}`}>{(r.overtimeMin / 60).toFixed(2)}</td>
                          <td className="text-right tabular-nums">{fmtCurrency(r.salesTotal, "USD")}</td>
                          ...
                          <td className="text-right">
                            <Button asChild variant="ghost" size="sm">
                              <Link to="/shifts" search={{ id: r.id }}>Details</Link>
                            </Button>
                          </td>
                        </tr>
                      );
...
function ShiftDetail({ id }: { id: string }) {
  ...
  const totalMin = Math.max(0, Math.round(((outD?.getTime() ?? Date.now()) - inD.getTime()) / 60000));
  const workedMin = Math.max(0, totalMin - (entry.break_minutes ?? 0));
  const overtimeMin = Math.max(0, workedMin - 8 * 60);
  const sales = salesQ.data ?? [];
  const salesTotal = sales.reduce((a, b) => a + Number(b.total ?? 0), 0);
  const name = entry.profiles?.full_name || `${entry.profiles?.first_name ?? ""} ${entry.profiles?.last_name ?? ""}`.trim() || entry.profiles?.email;
  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        ...
        <Card>
          <CardHeader><CardTitle>{name}</CardTitle><CardDescription>Employee ID {entry.profiles?.employee_id} · {format(inD, "EEEE, MMMM d, yyyy")}</CardDescription></CardHeader>
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Sales during shift</CardTitle><CardDescription>{sales.length} transactions · {fmtCurrency(salesTotal, "USD")}</CardDescription></CardHeader>
          <CardContent>
            {sales.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead className="text-left text-xs text-muted-foreground border-b">
                  <tr><th className="py-2">Receipt</th><th>Time</th><th>Status</th><th className="text-right">Total</th></tr>
                </thead>
                <tbody>
                  {sales.map((s) => (
                    <tr key={s.id} className="border-b last:border-0">
                      <td className="py-2 font-mono">#{(s as { receipt_number?: number }).receipt_number ?? s.id.slice(0, 6)}</td>
                      <td>{format(new Date(s.created_at), "p")}</td>
                      <td><Badge variant="outline">{s.status}</Badge></td>
                      <td className="text-right tabular-nums">{fmtCurrency(Number(s.total), "USD")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
function SessionDetail({ id }: { id: string }) {
  return (
    <>
      <PageHeader title="Register Session Report" subtitle={`Session ${id.slice(0, 8).toUpperCase()}`} />
      <div className="flex-1 overflow-y-auto p-6 space-y-4">
        <Button asChild variant="ghost" size="sm" className="print:hidden">
          <Link to="/shifts"><ArrowLeft className="size-4 mr-2" /> Back to shifts</Link>
        </Button>

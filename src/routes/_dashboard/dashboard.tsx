...
import { ResponsiveContainer, AreaChart, Area, XAxis, Tooltip, CartesianGrid, BarChart, Bar, YAxis } from "recharts";
...
function DashboardPage() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [salesRes, refundsRes, products, lowStock, store, openShifts, employeesToday] = await Promise.all([
        sb.from("sales").select("id,total,subtotal,tax,discount,created_at,payment_method,cashier_id,status").gte("created_at", week.toISOString()).order("created_at"),
        sb.from("refunds").select("total,created_at,refund_type").gte("created_at", today.toISOString()),
        sb.from("products").select("id,price,stock", { count: "exact" }),
        sb.from("products").select("id,name,stock,min_stock").lte("stock", 5).order("stock").limit(5),
      ...
      const cashSales = todays.filter((s) => s.payment_method === "cash").reduce((a, s) => a + Number(s.total), 0);
      const cardSales = todays.filter((s) => s.payment_method !== "cash").reduce((a, s) => a + Number(s.total), 0);
      const totalTax = todays.reduce((a, s) => a + Number(s.tax || 0), 0);
      const totalDiscount = todays.reduce((a, s) => a + Number(s.discount || 0), 0);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const refundAmount = (refundsRes.data ?? []).filter((r: any) => r.refund_type !== "void").reduce((a: number, r: any) => a + Number(r.total || 0), 0);
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const inventoryValue = (products.data ?? []).reduce((s: number, p: any) => s + Number(p.price) * Number(p.stock), 0);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      return {
        todayTotal, weekTotal, cashSales, cardSales, totalTax, totalDiscount,
        refundAmount, netRevenue,
        txCount: todays.length, itemsSold,
        bestSelling, busiestHour: busiestHour.count > 0 ? `${busiestHour.hour}:00` : "—",
        employeesWorked, openShiftCount,
        topProducts, hourlyChart, chart,
        productCount: products.count ?? 0, inventoryValue,
        lowStock: lowStock.data ?? [], currency: store.data?.currency ?? "USD",
      };
    },
  });
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          ...
          <Kpi icon={Percent} label="Discounts" value={fmtCurrency(data?.totalDiscount ?? 0, cur)} />
          <Kpi icon={TrendingUp} label="Net revenue" value={fmtCurrency(data?.netRevenue ?? 0, cur)} highlight />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <CardContent className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data?.chart ?? []}>
                  ...
                  <Tooltip contentStyle={{ background: "var(--color-card)", border: "1px solid var(--color-border)", borderRadius: 8, fontSize: 12 }} formatter={(v: number) => fmtCurrency(v, cur)} />
          ...
          <Card>
            <CardHeader><CardTitle className="text-base">Sales by hour · today</CardTitle></CardHeader>
            <CardContent className="h-64">
              {(data?.hourlyChart?.length ?? 0) === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={data!.hourlyChart}>
                    ...
                    <YAxis fontSize={11} />
                    <Tooltip formatter={(v: number) => fmtCurrency(v, cur)} labelFormatter={(h) => `${h}:00`} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <Card className="lg:col-span-2">
            <CardHeader><CardTitle className="text-base">Top products today</CardTitle></CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader><CardTitle className="text-base flex items-center gap-2"><Wallet className="size-4" /> Register status</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              ...
              <Link to="/register" className="block text-sm text-primary hover:underline">Open / close register →</Link>
              <Link to="/shifts" className="block text-sm text-primary hover:underline">All shift reports →</Link>
            </CardContent>
          </Card>
        </div>
...
function Kpi({ icon: Icon, label, value, highlight, small }: {
  ...
  return (
    <Card className={highlight ? "border-primary/40 bg-primary/5" : ""}>
      <CardContent className="p-4">
        ...
        <div className={`font-bold font-mono ${small ? "text-sm truncate" : "text-xl"}`}>{value}</div>
      </CardContent>
    </Card>
  );
}

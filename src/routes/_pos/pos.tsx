...
export function PosPage() {
  ...
  useEffect(() => {
    ...
    void purgeIfStoreChanged(store.id)
      .then((result) => {
        setStoreSwitchBlocked(result.blocked);
        if (result.blocked) {
          toast.error(
            `This terminal cannot switch stores while ${result.preservedUnsyncedRecords} financial record${result.preservedUnsyncedRecords === 1 ? " is" : "s are"} unsynced. Open Pending Sync or contact support.`,
            { duration: 12_000 },
          );
        }
      })
  ...
  const finalize = useMutation({
    ...
    // device is offline. This mutation must run so the local cash sale is
    ...
    mutationFn: async (payment: CompletedPayment) => {
      ...
      // 3. Prepare the complete transaction payload.
      ...
      if (saleErr || !sale?.id) {
        ...
        throw new SaleError(
          ...
          isStock
            ...
            : friendlyDbMessage(saleErr, "Unable to save the complete sale. Please try again."),
          saleErr,
        );
      }
      return { sale, payment };
...
function ProductTile({
  ...
  return (
    ...
    >
      ...
      {!url && (
        <div className="absolute inset-0 grid place-items-center bg-muted/35" aria-hidden="true">

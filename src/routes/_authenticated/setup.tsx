...
const DEFAULT_STATE = (): WizardState => ({
  step: 0,
  owner: { first_name: "", last_name: "", email: "", phone: "", accepted_terms: false },
  store: {
...
function SetupWizardPage() {
  ...
  const persist = async (next: WizardState, opts?: { complete?: boolean }) => {
    if (!store?.id) return;
    setSaving(true);
    try {
  ...
  const finish = async () => {
    ...
    try {
      ...
      await persist(state, { complete: true });
        ...
        {state.step > 0 && state.step < 11 && (
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              {state.step === 10 && (
                <Button onClick={async () => { await finish(); goTo(11); }} disabled={finishing}>
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      ...
      <div className="flex items-start gap-2 pt-2">
        <Checkbox id="terms" checked={o.accepted_terms} onCheckedChange={(v) => patch({ accepted_terms: !!v })} />
        <label htmlFor="terms" className="text-sm text-muted-foreground leading-tight">
          I accept the <a className="underline" href="#">Terms of Service</a> and <a className="underline" href="#">Privacy Policy</a>.
        </label>
      </div>
    </div>
  );

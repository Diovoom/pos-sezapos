  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (
          !payload.action ||
          !["clock_in", "clock_out", "start_break", "end_break"].includes(payload.action)
        ) {
        ...
        if (Number.isNaN(requested.getTime()))
          return json({ error: "Invalid time-clock timestamp" }, 400);
        const now = Date.now();
        if (requested.getTime() > now + 5 * 60_000)
          return json({ error: "Time-clock timestamp is in the future" }, 400);
        ...
        if (!existing) {
          if (payload.action === "clock_out")
            return json({ ok: true, entry: null, alreadyApplied: true });
        ...
        if (payload.action === "clock_out") {
          ...
          if (existing.break_start)
            return json({ ok: true, entry: existing, alreadyApplied: true });

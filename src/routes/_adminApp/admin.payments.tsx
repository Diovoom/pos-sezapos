  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Subscription invoices merchants pay to SEZA for the POS software. Customer checkout
            transactions are not shown here.
          </p>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        />
        <SummaryCard
          label="Collected by SEZA"
          value={money(summary?.collected_cents ?? 0)}
          icon={DollarSign}
          tone="text-emerald-600"
        />
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            New invoice.payment_succeeded and invoice.payment_failed webhooks are stored here
            permanently.
          </CardDescription>
          ...
          {query.isLoading ? (
            ...
            <div className="p-10 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                New Stripe invoice events appear here automatically. Current subscription status
                  ...
                  <tr key={payment.id} className="border-t hover:bg-muted/20">
                    <td className="p-3">
                      {payment.store ? (
                        <Link
                          ...
                          params={{ storeId: payment.store.id }}
                    ...
                    <td className="p-3">
                      <div className="font-semibold">
                        {money(
                          payment.amount_paid_cents || payment.amount_due_cents,
                          payment.currency,
                        )}
                      </div>
                      {payment.amount_due_cents !== payment.amount_paid_cents && (
                        <div className="text-xs text-muted-foreground">
                          Due {money(payment.amount_due_cents, payment.currency)}
                        </div>
                      )}
                    </td>
                    <td className="p-3">
                        ...
                        {payment.hosted_invoice_url && (
                          <Button asChild size="sm" variant="outline">
                            <a href={payment.hosted_invoice_url} target="_blank" rel="noreferrer">
                              Invoice <ExternalLink className="ml-1 h-3 w-3" />
                            </a>
                          </Button>
                        )}
                        {payment.invoice_pdf_url && (
                          <Button asChild size="sm" variant="ghost">
                            <a href={payment.invoice_pdf_url} target="_blank" rel="noreferrer">
                              PDF
                            </a>
                          </Button>
                        )}
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="text-muted-foreground">
          {query.data?.count ?? 0} payment record{(query.data?.count ?? 0) === 1 ? "" : "s"}
        </span>

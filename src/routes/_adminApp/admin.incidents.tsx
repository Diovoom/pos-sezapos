    ...
    try {
      await transition({
        data: {
          ...
          resolutionCode: "incident_resolved",
          reason: "Resolved from Incident Center",
        },
      });
      ...
      toast.error(error?.message ?? "Could not resolve incident");
    } finally {
      setBusy(false);
    }
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Resolve urgent merchant problems and review failed SEZA subscription payments.
          </p>
        </div>
      ...
      {query.isLoading ? (
        ...
        <Card className="border-destructive">
          <CardContent className="p-6">
            <div className="text-sm text-destructive">
              {(query.error as any)?.message ?? "Could not load incidents"}
            </div>
        ...
        <>
          <div className="grid gap-3 sm:grid-cols-3">
            <Stat icon={AlertTriangle} label="Active incidents" value={tickets.length} />
            <Stat
              icon={LifeBuoy}
              label="Urgent support"
              value={tickets.filter((ticket: any) => ticket.priority === "urgent").length}
            />
            <Stat icon={CreditCard} label="Billing alerts" value={billing.length} />
          </div>

          <Card>
            <CardHeader>
              <CardTitle>Urgent merchant incidents</CardTitle>
              <CardDescription>
                These remain active until an admin records a resolution.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {tickets.length === 0 ? (
                <div className="flex items-center justify-center gap-2 py-10 text-sm text-muted-foreground">
                  <CheckCircle2 className="h-5 w-5 text-emerald-600" /> No active urgent incidents.
                </div>
              ) : (
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {ticket.store?.name ?? "Unknown merchant"} · Updated{" "}
          ...
          <Card>
            <CardHeader>
              <CardTitle>Merchant billing alerts</CardTitle>
              <CardDescription>
                Failed subscription payments to SEZA. These are not customer checkout transactions.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {billing.length === 0 ? (
                <div className="py-8 text-center text-sm text-muted-foreground">
                  No failed merchant subscription payments in this window.
                </div>
              ) : (
                billing.map((alert: any) => (
                  <div
                    key={alert.id}
                  ...
                  >
                    <div>
                      <div className="font-medium">
                        {alert.store?.name ?? "Unmatched Stripe customer"}
                      </div>
                      <div className="text-xs text-muted-foreground">
                        {alert.failure_message || alert.status} ·{" "}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button asChild size="sm" variant="outline">
                        <Link to="/admin/payments">Review payment</Link>
                      </Button>
                    </div>
                  </div>
                ))
      ...
      <Dialog open={!!resolveTarget} onOpenChange={(open) => !open && setResolveTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Record the actual fix. The case will move to Resolved and remain available in history.
            </DialogDescription>
          </DialogHeader>
          <Field label="Resolution summary">
            <Textarea
              ...
              placeholder="What failed, what was changed, and how you verified the fix…"
            />
          </Field>
          <DialogFooter>
            ...
            <Button disabled={busy || summary.trim().length < 5} onClick={resolveIncident}>

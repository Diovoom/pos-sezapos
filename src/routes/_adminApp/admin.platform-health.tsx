  ...
  const services = data
    ? [
        ...
        },
        {
          ...
          detail: data.supabase.configured
            ? "URL, publishable key, and service role are configured"
        ...
        },
        {
          ...
          detail: data.email.configured
            ? "Email API and send endpoint configured"
            : "Email API or send endpoint missing",
        },
      ]
    : [];
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Live, non-secret checks for the services SEZA Admin depends on.
          </p>
        </div>
        <Button
          variant="outline"
            ...
            <div className="divide-y rounded-lg border">
              {services.map((service) => {
                ...
                return (
                  <div
                    key={service.label}
                    className="grid gap-2 p-4 sm:grid-cols-[220px_100px_1fr] sm:items-center"
                  >
                    <div className="flex items-center gap-2 font-medium">
                      <Icon className="h-4 w-4 text-muted-foreground" /> {service.label}
                    </div>
                    <Badge variant={service.ok ? "default" : "destructive"} className="w-fit">
                      {service.ok ? "Ready" : "Needs attention"}
                    </Badge>

    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const blocked = await guardApiRequest(request, {
          scope: "api.email.transactional_send",
          limit: 30,
          windowSeconds: 60,
          blockSeconds: 300,
          maxBodyBytes: 131072,
        ...
        const userLimit = await consumeRateLimit({
          scope: "api.email.transactional_send.user",
          limit: 20,
          windowSeconds: 60,
          blockSeconds: 300,
          identifier: user.id,
        ...
        if (suppressionError) {
          ...
          return jsonResponse({ error: "Failed to verify suppression status" }, { status: 500 });
        }
        ...
        if (tokenLookupError) {
          ...
          return jsonResponse({ error: "Failed to prepare email" }, { status: 500 });
        }

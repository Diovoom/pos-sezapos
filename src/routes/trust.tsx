...
import { Shield, Lock, FileCheck, Activity, HeartHandshake, BookOpen, Database, AlertCircle } from "lucide-react";

export const Route = createFileRoute("/trust")({
  head: () => ({
    meta: [
      { title: "Trust Center — SEZA POS" },
      { name: "description", content: `${LEGAL_CONFIG.companyName}'s Trust Center: security practices, compliance posture, system status, privacy, incident response, and responsible disclosure.` },
      { property: "og:title", content: "SEZA POS — Trust Center" },
      { property: "og:description", content: "Security, compliance, status, privacy, and incident response." },
    ],
  }),
  component: TrustPage,
});
  ...
  return (
    <MarketingShell>
      <section className="max-w-3xl mx-auto px-6 py-16 text-center">
        ...
        <h1 className="mt-3 text-4xl md:text-5xl font-bold tracking-tight">How we earn and keep your trust.</h1>
        <p className="mt-5 text-lg text-muted-foreground">
          This page is maintained by {LEGAL_CONFIG.companyName} to summarize the security, privacy, and reliability practices that underpin {LEGAL_CONFIG.productName}. It is app-owned content and is not an independent certification.
        </p>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-16 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {TILES.map((t) => {
          ...
          if ("to" in t && t.to) return <Link key={t.title} to={t.to} className={cls}>{inner}</Link>;
          if ("slug" in t && t.slug) return <Link key={t.title} to="/legal/$slug" params={{ slug: t.slug }} className={cls}>{inner}</Link>;
          if ("mail" in t && t.mail) return <a key={t.title} href={`mailto:${LEGAL_CONFIG.securityEmail}`} className={cls}>{inner}</a>;
          return <div key={t.title} className={cls}>{inner}</div>;
        })}
      </section>

      <section className="max-w-3xl mx-auto px-6 py-8 text-sm text-muted-foreground text-center border-t">
        Questions about our security or compliance posture? Contact <a className="text-primary underline" href={`mailto:${LEGAL_CONFIG.securityEmail}`}>{LEGAL_CONFIG.securityEmail}</a>.
      </section>
    </MarketingShell>
  );
}

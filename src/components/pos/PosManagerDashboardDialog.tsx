...
import { Clock, Cloud, LogOut, Receipt, RotateCcw, Settings, Users, X } from "lucide-react";
    ...
    <DialogContent className="h-[94dvh] max-w-6xl overflow-hidden p-0">
      ...
      <div className="h-full overflow-y-auto p-4 pb-24">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <button type="button" onClick={()=>setHardware(v=>!v)} className="rounded-xl border bg-background p-4 text-left hover:border-primary/50">
        ...
        {hardware && <div className="mt-5 grid gap-4 lg:grid-cols-2">
          <NativeUsbPrinterPanel/><NativeDrawerPanel/><NativeScannerPanel/>{storeId?<NativeCustomerDisplayPanel storeId={storeId} storeName={storeName}/>:null}
        </div>}
      </div>
    </DialogContent>
  </Dialog>;

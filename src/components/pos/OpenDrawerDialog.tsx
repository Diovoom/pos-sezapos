...
export type DrawerReason = "make_change" | "count_shift" | "safe_drop" | "manager_request" | "other";
...
const REASONS: { value: DrawerReason; label: string; description: string }[] = [
  ...
  { value: "count_shift", label: "Count Shift", description: "Begin the end-of-shift count and close." },
  { value: "safe_drop", label: "Cash Pickup / Safe Drop", description: "Remove cash from the drawer to the safe." },
  { value: "manager_request", label: "Manager Request", description: "Requires manager or owner PIN." },
  ...
  const submit = useMutation({
    ...
    onSuccess: (r) => {
      if (r.status === "opened") {
        ...
        toast.error("Cash drawer unavailable. Connect a supported register bridge or receipt printer.");
      }
    ...
    <>
      <Dialog open={open} onOpenChange={(v) => { if (!submit.isPending) onOpenChange(v); }}>
        <DialogContent className="sm:max-w-md">
          ...
          <RadioGroup value={reason} onValueChange={(v) => setReason(v as DrawerReason)} className="gap-2">
            {REASONS.map((r) => (
              <label
                key={r.value}
              ...
              >
                <RadioGroupItem value={r.value} id={`drawer-reason-${r.value}`} className="mt-0.5" />
          ...
          <div className="space-y-1">
            ...
            <Textarea
              ...
              placeholder={requiresNote ? "Required — describe why the drawer needs to open" : "Optional"}
            />
          </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRequest} disabled={submit.isPending || !amountValid || !noteValid}>
      ...
      <ManagerOverrideDialog
        ...
        onApprove={(r) => { setApprover(r); setTimeout(() => submit.mutate(), 0); }}
      />
    </>
  );
}

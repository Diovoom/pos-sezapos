...
import { CircleCheck, Headphones, Loader2, MessageCircle, Phone, Send, User, X } from "lucide-react";
...
export function WebsiteLiveChat() {
  ...
  const poll = useCallback(async (session?: StoredSession | null) => {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setChat((current) => current ? { ...current, messages: data.messages || current.messages } : current);
    } catch (sendError: any) {
      setDraft(message);
  ...
  async function endChat() {
    ...
    if (!window.confirm("End this live chat? The conversation will stay saved for SEZA Support.")) return;
    setBusy(true);
    try {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-slate-950/45 p-3 sm:items-center sm:p-6" role="dialog" aria-modal="true" aria-label="SEZA live chat">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="truncate text-xs text-blue-100">
                {chat ? (isEnded ? "Chat ended" : agentJoined ? "Connected with support" : "Waiting for an agent") : "Name and phone are required before connecting"}
              </div>
            </div>
          </div>
          <button type="button" onClick={() => setOpen(false)} className="grid size-9 shrink-0 place-items-center rounded-full border border-white/25 text-white hover:bg-white/10" aria-label="Close live chat">
            <X className="size-4" />
          </button>
        </header>
            ...
            <div className="mt-5 space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input id="seza-chat-name" value={name} onChange={(event) => setName(event.target.value)} className="pl-9" autoComplete="name" placeholder="Your name" />
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input id="seza-chat-phone" value={phone} onChange={(event) => setPhone(event.target.value)} className="pl-9" inputMode="tel" autoComplete="tel" placeholder="(555) 555-5555" />
                </div>
              </div>
              <div className="hidden" aria-hidden="true">
                <Label htmlFor="seza-chat-website">Website</Label>
                <Input id="seza-chat-website" tabIndex={-1} value={website} onChange={(event) => setWebsite(event.target.value)} autoComplete="off" />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="seza-chat-message">How can we help?</Label>
                <Textarea id="seza-chat-message" value={openingMessage} onChange={(event) => setOpeningMessage(event.target.value)} rows={5} placeholder="Tell us about setup, pricing, hardware, your account, or a POS issue…" />
              </div>
              {error && <p className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</p>}
              <Button className="h-12 w-full rounded-full bg-blue-700 font-bold text-white hover:bg-blue-800" disabled={busy} onClick={startChat}>
          ...
          <>
            <div className="border-b border-blue-100 bg-blue-50 px-5 py-3 text-xs text-blue-900 dark:border-blue-400/15 dark:bg-blue-500/10 dark:text-blue-100">
              <div className="flex items-center justify-between gap-3">
                <span>Chat #{chat.ticketNumber ?? "—"} · {chat.visitorName}</span>
                {!isEnded && <button type="button" onClick={endChat} disabled={busy} className="font-bold text-blue-800 underline underline-offset-2 dark:text-blue-200">End chat</button>}
              </div>
            </div>
            <div className="flex-1 space-y-3 overflow-y-auto bg-slate-50 p-4 dark:bg-slate-950">
              {!agentJoined && !isEnded && (
                <div className="rounded-2xl border border-blue-200 bg-white p-3 text-sm text-blue-950 dark:border-blue-400/20 dark:bg-slate-900 dark:text-blue-100">
                  <div className="flex items-center gap-2 font-semibold"><Loader2 className="size-4 animate-spin" /> Waiting for a SEZA agent</div>
                  <p className="mt-1 text-xs text-muted-foreground">You can keep this window open or come back during this browser session.</p>
                </div>
              )}
              {chat.messages.map((message) => (
                <div key={message.id} className={cn("flex", message.from === "visitor" ? "justify-end" : "justify-start")}>
                  <div className={cn(
                    "max-w-[86%] rounded-2xl px-3.5 py-2.5 text-sm shadow-sm",
                    message.from === "visitor" ? "bg-blue-700 text-white" : "border border-blue-100 bg-white text-slate-950 dark:border-blue-400/15 dark:bg-slate-900 dark:text-white",
                  )}>
                    <div className={cn("mb-1 text-[10px]", message.from === "visitor" ? "text-blue-100" : "text-muted-foreground")}>
                      {message.from === "visitor" ? "You" : message.author || "SEZA Support"} · {new Date(message.createdAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}
                    </div>
              ...
              {isEnded && (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900 dark:border-emerald-400/20 dark:bg-emerald-500/10 dark:text-emerald-100">
                  <div className="flex items-center gap-2 font-bold"><CircleCheck className="size-4" /> This chat has ended</div>
                  <p className="mt-1 text-xs">The transcript remains saved with SEZA Support.</p>
                  <Button variant="outline" size="sm" className="mt-3" onClick={startAnotherChat}>Start another chat</Button>
                </div>
              )}
            ...
            {!isEnded && (
              <footer className="border-t bg-white p-3 dark:bg-slate-950">
                ...
                <div className="flex items-end gap-2">
                  ...
                  />
                  <Button size="icon" className="size-11 shrink-0 rounded-full bg-blue-700 hover:bg-blue-800" onClick={sendMessage} disabled={busy || !draft.trim()} aria-label="Send message">

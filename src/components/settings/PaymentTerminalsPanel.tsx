...
export function PaymentTerminalsPanel({ canEdit }: { canEdit: boolean }) {
  ...
  const terminals = useQuery({
    ...
    queryFn: async (): Promise<Terminal[]> => {
      const { data } = await sb.from("payment_terminals").select("*").eq("store_id", storeId).order("created_at");
  ...
  const add = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await sb.from("payment_terminals").insert({
        store_id: storeId,
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { toast.success("Removed"); qc.invalidateQueries({ queryKey: ["payment_terminals"] }); },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Failed"),
  });
  ...
  const toggle = useMutation({
    mutationFn: async (t: Terminal) => {
      ...
      const { error } = await sb.from("payment_terminals").update({
        status: nextStatus,
        last_seen_at: nextStatus === "active" ? new Date().toISOString() : t.last_seen_at,
      }).eq("id", t.id);
      if (error) throw error;
      void logAudit({ action: "terminal.toggle", entity: "payment_terminal", entity_id: t.id, details: { status: nextStatus } });
    },
              ...
              <tbody>
                {terminals.data.map((t) => (
                  <tr key={t.id} className="border-b last:border-0">
                    ...
                    <td>
                      <Badge variant="outline" className={t.status === "active" ? "text-success border-success/30" : "text-muted-foreground"}>
                        {t.status}
                      </Badge>
                    </td>
                    <td className="text-right">
                      <Button variant="ghost" size="sm" disabled={!canEdit} onClick={() => toggle.mutate(t)}>
                        {t.status === "active" ? "Deactivate" : "Activate"}
                      </Button>
                      <Button variant="ghost" size="sm" disabled={!canEdit} onClick={() => remove.mutate(t.id)}>
          ...
          <div className="border-t pt-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <Label>Label</Label>
                <Input value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="Front counter" />
              </div>
              <div className="space-y-1">
            ...
            <Button onClick={() => add.mutate()} disabled={add.isPending}>
              {add.isPending ? <Loader2 className="size-4 animate-spin mr-2" /> : <Plus className="size-4 mr-2" />} Add terminal
            </Button>
          </div>
        )}
      </CardContent>

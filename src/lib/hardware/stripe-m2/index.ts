...
const STRIPE_RUNTIME_KEY = "__sezaStripeTerminalRuntimeV2";

function runtimeState(): StripeRuntimeState {
  const root = globalThis as typeof globalThis & {
    [STRIPE_RUNTIME_KEY]?: StripeRuntimeState;
  };
...
async function installConnectionTokenListener(mod: StripeModule): Promise<void> {
  const state = runtimeState();
  if (!state.tokenListenerPromise) {
    state.tokenListenerPromise = mod.StripeTerminal
      .addListener(mod.TerminalEventsEnum.RequestedConnectionToken, () => {
        state.tokenDeliveryQueue = state.tokenDeliveryQueue
          .catch(() => undefined)
          .then(async () => {
      ...
      .then(() => undefined)
      .catch((error) => {
        state.tokenListenerPromise = null;
        throw error;
      });
  }
  await state.tokenListenerPromise;
}
...
async function probeNativeReader(mod: StripeModule): Promise<any | null> {
  try {
    ...
    runtimeState().nativeInitialized = true;
...
async function initialize(testMode: boolean) {
  ...
  if (!state.initializePromise) {
    state.initializePromise = (async () => {
      // Capacitor's Stripe plugin replaces its TokenProvider field every time
      // initialize() is called, even when Stripe's native Terminal singleton is
      // already initialized. Probe first so a second initialize can never break
      // the connection-token callback after a WebView/module reload.
      await probeNativeReader(mod);
      if (runtimeState().nativeInitialized) {
        state.initializedMode = testMode;
        return;
      }
...
export async function connectReader(driver: TerminalDriverId, onStatus?: (message: string) => void) {
  try {
    ...
    const failure = terminalError(error, "CONNECT_READER");
    if (typeof localStorage !== "undefined") {
      localStorage.setItem("pos.terminal.lastError", failure.message);
    }
...
export async function getNativeConnectedReader() {
  try {
    const mod = await loadModule();
...
export async function isReady(_driver: TerminalDriverId) {
  return Boolean(await getNativeConnectedReader());
}
...
export async function charge(
  ...
  try {
    ...
    const message = terminalError(error, "PAYMENT_FLOW").message;
...
export async function disconnect() {
  const state = runtimeState();
  const terminalId = state.connected?.terminalId;
  try {
    const mod = await loadModule();
    const existing = await probeNativeReader(mod);
    if (existing) await mod.StripeTerminal.disconnectReader();
  } catch {
    // Treat an already-disconnected or not-yet-initialized reader as disconnected.
  }
  state.connected = null;
  if (terminalId) {
    await updateStripeTerminal("disconnected", terminalId).catch(() => undefined);
  }
}

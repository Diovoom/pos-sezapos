...
const CMD = {
  ...
  kickDrawer: (pulseMs = 120) => {
    const ms = Math.max(20, Math.min(510, Math.round(pulseMs)));
    const t1 = Math.max(1, Math.min(255, Math.round(ms / 2)));
    return bytes(ESC, 0x70, 0x00, t1, 0xfa);
  },
};
...
function sanitizeReceiptText(value: string): string[] {
  return String(value ?? "")
    ...
    .map((line) =>
      line
        .replace(/\s+(?:ref|reference|transaction\s*(?:ref|reference|id)|payment\s*(?:ref|reference|id))\s*:\s*[^\s]+.*$/i, "")
        .trimEnd(),
    )
    .filter((line) => {

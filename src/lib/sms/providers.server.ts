...
async function sendTwilio({ to, body, credentials, senderId }: SendArgs): Promise<SmsSendResult> {
  ...
  if (!sid || !token)
    return { ok: false, error: "Twilio credentials missing (Account SID / Auth Token)" };
...
async function sendVonage({ to, body, credentials, senderId }: SendArgs): Promise<SmsSendResult> {
  ...
  try {
    ...
    if (!res.ok || message?.status !== "0") {
      return {
        ok: false,
        error: message?.["error-text"] || `Vonage error ${res.status}`,
        raw: json,
      };
    }
...
async function sendMessageBird({
  ...
  credentials,
  senderId,
}: SendArgs): Promise<SmsSendResult> {
  ...
  try {
    ...
    return res.ok
      ...
      : {
          ok: false,
          error: json?.errors?.[0]?.description || `MessageBird error ${res.status}`,
          raw: json,
        };
  } catch (err) {
    return { ok: false, error: err instanceof Error ? err.message : "Network error" };
...
async function sendPlivo({ to, body, credentials, senderId }: SendArgs): Promise<SmsSendResult> {
  ...
  if (!authId || !authToken)
    return { ok: false, error: "Plivo Auth ID and Auth Token are required" };
...
export async function sendViaProvider(
  ...
  switch (provider) {
    ...
    case "vonage":
      return sendVonage(args);
    case "messagebird":
      return sendMessageBird(args);
    case "plivo":
      return sendPlivo(args);
    default:

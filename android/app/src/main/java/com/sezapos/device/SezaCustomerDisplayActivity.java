...
public class SezaCustomerDisplayActivity extends Activity {
    ...
    private String html() {
        return String.join("",
            ...
            ".message{font-size:54px;line-height:1.05;font-weight:900;letter-spacing:-.035em;max-width:92vw;overflow-wrap:anywhere}",
            ".idlePhoto{display:block;max-width:46vw;max-height:48vh;object-fit:contain;border-radius:24px;box-shadow:0 18px 55px rgba(15,23,42,.14)}",
            ...
            ".paymentOnly{position:relative;width:100vw;height:100vh;background:#fff;overflow:hidden}",
            ".paymentGuideImage{position:absolute;inset:0;width:100%;height:100%;object-fit:contain}",
            ".paymentLiveTotal{position:absolute;left:50%;top:72%;transform:translateX(-50%);text-align:center;white-space:nowrap}",
            ".paymentLiveLabel{color:#64748b;font-size:24px;font-weight:700}",
            ".paymentLiveAmount{margin-top:4px;color:#0f172a;font-size:58px;font-weight:900;letter-spacing:-.04em}",
            ...
            "main=`<div class='paymentOnly'><img class='paymentGuideImage' src='file:///android_res/drawable/seza_card_reader_payment_guide.png' alt=''><div class='paymentLiveTotal'><div class='paymentLiveLabel'>Total</div><div class='paymentLiveAmount'>${money(p.total)}</div></div></div>`;",
            ...
            "}else{",
            "const idleScale=Math.max(.8,Math.min(1.8,Number(p.idleTextScale||1)));",
            "if(p.idleMode==='image'&&p.idleImageUrl){",
            "main=`<div class='center'><img class='idlePhoto' src='${esc(p.idleImageUrl)}' alt=''></div>`;",
            "}else{",
            "main=`<div class='center'><div class='message' style='font-size:${Math.round(54*idleScale)}px'>${esc(p.idleMessage||'Welcome')}</div></div>`;",
            "}",
            "}",
            "document.getElementById('app').innerHTML=phase==='awaiting_card'?main:`<div class='wrap'><div class='head'>${esc(p.storeName||'SEZA POS')}</div><div class='main'>${main}</div></div>`;",
            "};",
            "</script></body></html>"
        );
    }

...
function MerchantSupportChat() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: ticket, error: ticketError }, { data: notes, error: notesError }] = await Promise.all([
        (supabase.from as any)("support_tickets")
          .select("id,ticket_number,subject,status,priority,requester_id,requester_email,assigned_admin_id,chat_status,chat_ended_at,resolution_summary,resolution,created_at,updated_at,last_message_at")
          .eq("id", ticketId)
          .maybeSingle(),
  ...
  if (query.isLoading) {
    return <div className="mx-auto max-w-4xl p-6 text-sm text-muted-foreground"><Loader2 className="mr-2 inline h-4 w-4 animate-spin" /> Loading support chat…</div>;
  }
  if (query.isError || !query.data) {
    return <div className="mx-auto max-w-4xl p-6 text-sm text-destructive">Could not open this support case.</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-4 md:p-6">
      <Link to="/help" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
        <ArrowLeft className="h-4 w-4" /> Back to support
      </Link>
      <PageHeader title={`Support case #${ticket.ticket_number ?? "—"}`} subtitle={ticket.subject} />

      <div className="flex flex-wrap gap-2">
        <Badge variant="outline">{ticket.priority}</Badge>
        <Badge>{ticket.status}</Badge>
        <Badge variant={ended ? "secondary" : "default"}>{ended ? "Chat ended" : "Live chat"}</Badge>
      </div>

      {ticket.resolution_summary || ticket.resolution ? (
        <Card className="border-emerald-500/30 bg-emerald-500/5">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base"><CheckCircle2 className="h-4 w-4 text-emerald-600" /> SEZA resolution</CardTitle>
          </CardHeader>
          <CardContent className="whitespace-pre-wrap text-sm">{ticket.resolution_summary || ticket.resolution}</CardContent>
        </Card>
      ) : null}

      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><MessageSquare className="h-5 w-5" /> Live conversation</CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-3">
          {messages.length === 0 ? (
            ...
            const label = mine ? "You" : fromSeza ? "SEZA Support" : (item.author_email || "Merchant team");
            return (
              <div key={item.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] rounded-xl px-3 py-2 ${mine ? "bg-primary text-primary-foreground" : fromSeza ? "bg-emerald-500/10" : "bg-muted"}`}>
                  <div className={`mb-1 text-[11px] ${mine ? "text-primary-foreground/75" : "text-muted-foreground"}`}>
          ...
          {closed ? (
            <div className="rounded-lg bg-muted p-4 text-sm">
              This case is closed. Open a new ticket if you need help with a different or recurring problem.
            </div>
          ) : (
            <div className="space-y-2 border-t pt-4">
              {ended && (
                <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3 text-sm">
                  SEZA ended the previous live session. Sending a new message will reactivate the conversation.
                </div>
              )}
              <Textarea
                rows={4}

...
export const Route = createFileRoute("/_dashboard/help")({
  head: () => ({ meta: [{ title: "Support — SEZA POS" }, { name: "description", content: "Get help from the SEZA POS platform team." }] }),
  component: HelpPage,
});
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Describe the issue. Include screenshots via the web dashboard if helpful.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="space-y-1.5">
            <Label htmlFor="subj">Subject</Label>
            <Input id="subj" value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="e.g. Card reader disconnects" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea id="msg" rows={5} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Steps to reproduce, error messages, etc." />
          </div>
          <div className="flex gap-2">
            <Button
              disabled={busy || !subject.trim()}
              onClick={() => { setBusy(true); create.mutate(); }}
            >
              {busy ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Send className="mr-2 size-4" />}
              Submit ticket
            </Button>
            <Button variant="outline" asChild>
              <a href="mailto:support@sezapos.com"><Mail className="mr-2 size-4" /> Email us</a>
            </Button>
            <Button variant="outline" asChild>
              <a href="https://status.sezapos.com" target="_blank" rel="noreferrer"><Globe className="mr-2 size-4" /> Status page</a>
            </Button>
          </div>
        </CardContent>
      </Card>

...
if (pkg.version !== publicVersion.version || pkg.version !== gradleVersion) {
  failures.push(`Version mismatch: package=${pkg.version}, public=${publicVersion.version}, Android=${gradleVersion}`);
}
...
requireText("src/routes/_pos/pos.tsx", 'networkMode: "always"', "offline-capable checkout mutation");
...
requireText("src/lib/offline/sync.ts", "receipt deferred until sale syncs", "receipt dependency ordering");
requireText("src/lib/offline/db.ts", 'db.transaction("meta", "readwrite")', "transaction-safe local receipt sequence");
requireText("src/lib/offline/db.ts", 'store_switch_conflict', "store-switch financial safety block");
requireText("src/lib/audit-log.ts", "store_id", "tenant-scoped audit rows");
requireText("android/app/src/main/AndroidManifest.xml", 'android:allowBackup="false"', "Android backup disabled");
requireText("android/app/src/main/java/com/sezapos/app/MainActivity.java", "SezaSecureStoragePlugin.class", "secure-storage plugin registration");
requireText("capacitor-shell/lib/pairing.ts", "SecureStorage.savePairing", "Keystore-backed pairing persistence");
requireText("supabase/migrations/20260723212000_tenant_permission_scope.sql", "ur.store_id = public.current_store_id()", "tenant-scoped permission migration");
requireText("supabase/migrations/20260723213000_atomic_pos_sale_finalize.sql", "CREATE OR REPLACE FUNCTION public.finalize_pos_sale", "atomic sale migration");
requireText("supabase/migrations/20260723213000_atomic_pos_sale_finalize.sql", "FOR UPDATE OF p", "inventory row locking");
requireText("supabase/migrations/20260723214500_support_and_platform_consistency.sql", "supabase_realtime", "support realtime migration");
forbidText("src/components/i18n/GlobalLanguageRuntime.tsx", "replaceTextNodeBySubstring", "substring translator");
...
if (zxing?.engines?.node && String(zxing.engines.node).includes(">= 24")) {
  ...
  if (!nvm.startsWith("24")) failures.push(".nvmrc must use Node 24 because the locked ZXing package requires Node >=24");
}

if (!fs.existsSync(path.join(root, "supabase/migrations"))) failures.push("Missing Supabase migrations directory");
if (!fs.existsSync(path.join(root, ".github/workflows/quality.yml"))) warnings.push("GitHub quality workflow is missing");
...
console.log(`SEZA production verification passed for v${pkg.version} build ${publicVersion.build}.`);
